...
const lineData = [
  { value: 30 }, { value: 45 }, { value: 35 }, { value: 60 }, { value: 50 }, { value: 70 }, { value: 65 }
];

const barData = [
  { value: 40 }, { value: 65 }, { value: 45 }, { value: 80 }, { value: 60 }, { value: 90 }
];

const areaData = [
  { value: 20 }, { value: 40 }, { value: 30 }, { value: 50 }, { value: 45 }, { value: 65 }, { value: 60 }, { value: 75 }
];
...
const MetricCard = ({ title, value, change, delay }: MetricCardProps) => (
  <motion.div
...
const ChartPanel = ({ delay, rotation, position, children }: ChartPanelProps) => (
  <motion.div
...
export default function DataVisualizationBackground() {
  return (
    <div className="relative w-full h-full bg-gradient-to-br from-slate-900 via-slate-800 to-teal-900 overflow-hidden">
          ...
          <div className="w-32 h-32 bg-gradient-to-br from-slate-800/90 to-slate-900/90 backdrop-blur-md border border-cyan-500/30 rounded-xl p-3 shadow-2xl shadow-cyan-500/30 flex flex-col justify-center items-center">
            ...
            <div className="flex gap-1">
              {[40, 60, 45, 70, 55].map((height, i) => (
                <motion.div
                  key={i}
                  className="w-3 bg-gradient-to-t from-cyan-500 to-teal-400 rounded-sm"
                  initial={{ height: 0 }}
